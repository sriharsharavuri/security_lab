...
def reencrypt(key,sPlain):
	recipher_pairs = []
	cipherArray = sPlain.split()
	for i in range(0, len(cipherArray), 2):
			c = int(cipherArray[i])
			d = int(cipherArray[i+1])
			y = random.randint( 0, key.p )
			c = (c*modexp(key.g,y,key.p))% key.p
			d = (d*modexp(key.h,y,key.p))% key.p
			recipher_pairs.append( [c, d] )

	encryptedStr = ""
	for pair in recipher_pairs:
			encryptedStr += str(pair[0]) + ' ' + str(pair[1]) + ' '
